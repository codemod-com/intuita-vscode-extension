...
const packageSettingsCodec = buildTypeCodec({
	...
	dependencies: t.union([t.record(t.string, t.string), t.undefined]),
});
...
const getDependencyVersion = (
	...
	packageSettings.dependencies?.[dependencyName] ??
	packageSettings.devDependencies?.[dependencyName] ??
	null;
	...
	async showInformationMessagesAboutUpgrades() {
		...
		for (const packageSettingsUri of uris) {
			...
			for (const [dependencyName, dependencyNewVersion] of dependencies) {
				const dependencyOldVersion = getDependencyVersion(
					dependencyName,
					validation.right,
				);
