...
export const destructIntuitaFileSystemUri = (
    ...
    if (!startArray) {
        throw new Error('Cannot destruct an URI that does not match the start pattern');
    }
    ...
    if (!endArray) {
        throw new Error('Cannot destruct an URI that does not match the end pattern');
    }

    const endLength = endArray[0]?.length ?? 0;

    const fsPath = uri.fsPath.slice(
        startLength,
        uri.fsPath.length - endLength,
    );
