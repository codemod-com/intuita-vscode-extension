...
export class IntuitaProvider implements WebviewViewProvider {
	...
	private __getTreeByCase = async (element: Element): Promise<TreeNode> => {
		...
		if (element.kind === ElementKind.CASE) {
			mappedNode.children = await Promise.all(element.children
	...
	private async __onClearStateMessage() {
		...
		const tree =
			this.__viewBreakdown === MessageKind.caseBreakdown
				? await this.__getTreeByCase(rootElement)
	...
	private __buildJobTree = async (element: JobElement): Promise<TreeNode> => {
		const mappedNode: TreeNode = {
			id: element.hash,
			kind: 'jobElement',
			actions: [
		...
		const isJobStale = await this.__jobManager.isJobStale(element.jobHash);
