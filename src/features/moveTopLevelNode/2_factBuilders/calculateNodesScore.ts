...
export const calculateKindScore = (
	...
	const leftNodeScores = leftNodes.map((leftNode) => {
		const leftKindIndex = kindOrder.indexOf(leftNode.kind);

		return Math.max((leftKindIndex - rightKindIndex) / kindOrder.length, 0);
	});
...
export const calculateNodesScore = (
	...
	const modifierScores = nodes.map((_, rightIndex) => {
		return calculateModifierScore(nodes, rightIndex, modifierOrder);
	});

	const kindScores = nodes.map((_, rightIndex) => {
		return calculateKindScore(nodes, rightIndex, kindOrder);
	});

	return [calculateAverage(modifierScores), calculateAverage(kindScores)];
};
